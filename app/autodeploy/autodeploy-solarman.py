...
import datetime
def read_config():
    global g_repoPath,g_mvn1,g_mvn2,g_serverIp,g_serverUser,g_serverPwd,g_cmd1,g_cmd2,g_cmd3,g_cmd4,g_targetFileName,g_targetFileName1,g_createFileName,g_createFileName1,g_localMkDirPath,g_localMkDirPath1,g_remoteFileMkDir,g_remoteFileMkDir1,remoteFilePath,remoteFilePath1,localFilePath1,remoteFilePath3
    cf=ConfigParser.ConfigParser()
    cf.read("b_config.ini")
...
def start_service(cmd1,cmd2,cmd3,cmd4):
    ...
    #如果存在，则杀死进程
    if len(output11)!=0:
        print("tomcat running,stop service......")
        for i in output11:
            #杀死进程
            print i
        ...
        if (cmd3) in output3:
            print("service is not stop！！！")
            #退出进程
            sys.exit(3)
        else:
            print("service stop sucessfully")
    else:
        print("no")
    ...
    print("check is tomcat  runnning")
    output4 = remoteRun(cmd2)

    #如果存在，运行成功
    if (cmd3) in output4:
        print("tomcat service run sucessfully")
    else:
        print('service is not running sucessfully！')
        sys.exit(3)
#远程执行linux命令
...
if __name__ == '__main__':
    ...
    #  mkdir()
    # ftp_upload_file()
    # ssh_clinet(g_serverIp,g_serverUser,g_serverPwd,g_cmd,g_path)
    # print datetime.datetime.now().strftime("%Y.%m.%d-%H:%M:%S")
