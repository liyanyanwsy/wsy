...
def guanggao():
    try:
        ...
        conn = MySQLdb.connect(host='192.168.1.53', user='zhouwei', passwd='Zhouwei@123', db='advert',
                               port=3306, charset='utf8')
                ...
                for ch in channel:
                    for so in soft_version:
                        for ha in hare_version:
                            ...
                            so1=so.decode('utf-8')
                            ha1=ha.decode("utf-8")
                            print pl1,po1,ch1,so1,ha1
                            url1="http://192.168.1.58:28080/advert/search?platform=%d&position=%d&runType=16&channel=%d&softversion=%s&hardversion=%s" %(pl1,po1,ch1,so1,ha1)
                ...
                for q in ios_channel:
                    for r in ios_soft_version:
                        for s in ios_hard_version:
                            ...
                            url1="http://192.168.1.58:28080/advert/search?platform=%d&position=%d&runType=16&channel=%d&softversion=%s&hardversion=%s" %(o1,p1,q1,r1,s1)
                ...
                for pro_ch in channel:
                    for pro_so in pro_soft_version:
                        for pro_ha in pro_hare_version:
                            ...
                            url1="http://192.168.1.58:28080/advert/search?platform=%d&position=%d&runType=16&channel=%d&softversion=%s&hardversion=%s" %(pro_pl1,pro_po1,pro_ch1,pro_so1,pro_ha1)
                            # url1="https://apic-cdn.solarman.cn/advert/search?platform=%d&position=%d&runType=16&channel=%d&softversion=%s&hardversion=%s" %(pl1,po1,ch1,so1,ha1)
                            req1=re.get(url1,headers=headers,verify=False)
                            print req1.status_code
                            print req1.content
                ...
                for pro_ios_ch in ios_channel:
                    for pro_ios_so in pro_ios_soft_version:
                        for pro_ios_ha in pro_ios_hard_version:
                            ...
                            url1="http://192.168.1.58:28080/advert/search?platform=%d&position=%d&runType=16&channel=%d&softversion=%s&hardversion=%s" %(pro_ios_pl1,pro_ios_po1,pro_ios_ch1,pro_ios_so1,pro_ios_ha1)
                            # url1="https://apic-cdn.solarman.cn/advert/search?platform=%d&position=%d&runType=16&channel=%d&softversion=%s&hardversion=%s" %(pl1,po1,ch1,so1,ha1)
                            req1=re.get(url1,headers=headers,verify=False)
                            print req1.status_code
                            print req1.content
