...
# Authorization=auth()
# url='http://www.f-qa.igen/maintain_s/operating/system/1448'
...
def first():
    Authorization=auth()
    url='http://www.f-qa.igen/maintain_s/operating/system/24143'
    headers={"authorization":"%s"%Authorization,
             "User-agent":"okhttp/3.9.1"}
    response=requests.get(url,headers=headers)
    result=json.loads(response.content)
    ...
    if grid_increment<buy_increment and charge_increment<discharge_increment:
        ...
        if useValue>0:
            ...
            buyRatio=None
            useDischargeRatio=None
    # print('首条的自发自用增量为%s,首条的自发充电增量为%s，首条的自发购电增量为%s，首条发电量为%s，首条用电量为%s，首条购电量为%s,首条充电量为%s'%(self_use,self_charge,buy_use,generationValue,useValue,buyValue,chargeValue))
...
while i<287:
    ...
    if grid_increment2>=buy_increment2 and charge_increment2>=discharge_increment2:
        ...
        if (generationValue2-generationValue)>0:
            #自发自用比例
            useRatio=min(self_usetotal+self_use,useValue2-useValue,generationValue2-generationValue)/(generationValue2-generationValue)
            #自发充电比例
            chargeRatio=min(self_chargetotal+self_charge,chargeValue2-chargeValue,generationValue2-generationValue)/(generationValue2-generationValue)
            if useRatio+chargeRatio>1:
                chargeRatio=1-useRatio
            else:
                #自发并网比例
            ...
            chargeRatio=None
            gridRatio=None
        if (useValue2-useValue)>0:
            #用电来源于发电量比例
            generationRatio=min(self_usetotal+self_use,useValue2-useValue,generationValue2-generationValue)/(useValue2-useValue)

            #用电来源于购电比例
            buyRatio=min(buy_usetotal+buy_use,useValue2-useValue,buyValue2-buyValue)/(useValue2-useValue)
            if generationRatio+buyRatio>1:
                buyRatio=1-generationRatio
            else:
                #用电量来源于放电量比例
